...
function displayStats(stats) {
  ...
  document.getElementById(
    "qr_code_scans_today"
  ).innerText = latestStats.qr_code_scans_today.toLocaleString();
  document.getElementById(
    "manual_entries_today"
  ).innerText = latestStats.manual_entries_today.toLocaleString();
  document.getElementById(
    "people_with_bluetooth_tracing_active_today"
  ).innerText = latestStats.people_with_bluetooth_tracing_active_today.toLocaleString();

  document.getElementById(
    "all_time_app_registrations"
  ).innerText = latestStats.all_time_app_registrations.toLocaleString();
  document.getElementById(
    "all_time_posters_created"
  ).innerText = latestStats.all_time_posters_created.toLocaleString();
...
function updateGraph() {
  ...
  let todaysQRCodeScans = todaysStats.map(data => {
    return { x: new Date(data.generated), y: data.qr_code_scans_today };
  });

  let todaysManualEntries = todaysStats.map(data => {
    return { x: new Date(data.generated), y: data.manual_entries_today };
  });
...
function updateHistoricGraph() {
  ...
  historicQRCodeScans.push({
    x: new Date(),
    y: latestStats.qr_code_scans_today
  });
  ...
  if (chartHistoric == null) {
    chartHistoric = new Chart(document.getElementById("chartHistoric"), config);
  } else {
